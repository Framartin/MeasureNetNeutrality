...
As some countries are over-represented, numbers below are not necessarily a good representation of the proportion of worldwide Internet connections subject to traffic shaping
</section>
...
Number IP | Number of distinct IP adresses | -
...
* *Up speed reduction rate* and *Down speed reduction rate* give a idea of the drop size
  * It makes sense only when Traffic Shaping rate is significant
* To estimate the reliability of these results, look at the number of distinct IP adresses and tests
...
We cannot directly compute the mean of tests by country, because some IP adresses have done more than 3000 tests. So it leads to a biais.

Instead we compute means of binary variables by IP adress, and then we agregate by country.

For example, if we have some errors and for 3 tests Shaperprobe have detected 2 without traffic shaping and one with, this IP adress have a mean of 1/3.
...
* if an ISP have an _Up or down shape rate_ lower than approximately 30%, _we cannot know_ if an ISP has implementing traffic shaping <small>It can be the results of errors, or that trafic shaping is implemented in only a part of the ISP's network</small>
...
In the maps below, colors represents the rate of the reduction of the upload speed due to traffic shaping relative to the capacity of the connection. This is the _intensity_ of Traffic Shaping.

It makes sense only when Traffic Shaping is detected, so the size of circles are the rate of traffic shaping detected in upload.
...
Correction term = False Negative - False Positive = <em> `r round(100*(RateFalseNegative-RateFalsePositive), digits = 3)` </em>
</section>
...
<section>
## Days of the week and traffic shaping

Below we have represented two calendars heat map of the rate of traffic shaping detected for each day : one in upload and another one in download. Keep in mind that there were just a few tests at the begin, so we can observe a lot of variations.
</section>
...
<br/>
In other words, is traffic shaping helping to manage networks ? The classical argument in favor of traffic shaping is that it handles network « jam ».
</section>
...
Thanks to every contributors of theses softwares:
