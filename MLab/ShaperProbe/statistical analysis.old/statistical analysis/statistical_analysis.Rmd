...
1. connection with the closest available MLab's server
...
If you want to make your own data analysis, you can download data in a cleaned csv file [here](http://respectmynet.eu/) **TODO**. 
...
Relation between days of the week and traffic shaping
-------------------------
...
```{r calendarheatmapsratets, echo=FALSE}
...
These graphical representations are quite simple, and this question should be analysed in more details.


Relation between capacity and traffic shaping
-------------------------

Is there a relationship between traffic shaping and capacity of connection ? In other words, could we determine if traffic shaping help to manage networks ? The classical argument in favor of traffic shaping is that it handles network « jam ».
...
Below there is the mean TODO . But we prefer to look at the median, because it's more robust to outliers.
...
Réduire à une plus petite période ? Faire depuis 2009 n'a pas de sens
ou visualisation dans le temps
...
TODO : évolution au cours du temps. On voit que pour les 1ers tests quelques vitesses très élevées : ils étaient à coté des serveurs

```{r countrymapcapacity, fig.width=12, fig.height=9, echo=FALSE, message=FALSE, warning=FALSE, error=FALSE}
#par(mai=c(0,0,0.2,0),xaxs="i",yaxs="i")
